...
const users = [ 'baji', 'ganesh', 'manoj', 'vamsi', 'dilip', 'Rehman' ];
const pick_Up = users.filter(value => value[0] === 'x');
console.log(pick_Up); 

const another_pick = users.filter(value => value[0] === 'g');
console.log(another_pick);

const third_pick = users.filter(value => value[0] === 'm');
